...
def get_eigenvalues(k):
    eigenvalues_here = []
    for tup in tools.generate_all_integer_tuples_below(5, dimensions):
...
if dimensions == 1:
    lattice = pc.BravaisLattice([num.array([1])])
...
k_track = tools.interpolate_vector_list(k_track, 49)

bdfile = file(",,band_diagram_analytic.data", "w")
evlists = [get_eigenvalues(k) for k in k_track]
